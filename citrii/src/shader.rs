...
unsafe fn create_shader(code: &str, shader_type: GLenum) -> GLuint {
    ...
    if status != gl::TRUE as GLint {
        ...
        panic!("Shader error: {}", std::str::from_utf8(&buf[0 .. buf_len as usize]).expect("Shader errorception"));
    }
    handle
}
    ...
    pub fn new(vertex_shader: &str, fragment_shader: &str) -> Shader {
        unsafe {
            ...
            if status != gl::TRUE as GLint {
                ...
                panic!("Shader link error: {}", std::str::from_utf8(&buf[0 .. buf_len as usize]).expect("Shader errorspection"));
            }
            gl::DeleteShader(vs);
            gl::DeleteShader(fs);
